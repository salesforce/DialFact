...
class DataLoaderLines(object):
    ''' For data iteration '''

    def __init__(self, lines, tokenizer, deviceid=0, max_len=256, cuda=True, batch_size=64):
        self.cuda = cuda
...
#         self.threshold = args.threshold
#         self.data_path = data_path
        inputs, ids, evi_list = self.read_lines(lines)
        self.inputs = inputs
        self.ids = ids
        self.evi_list = evi_list
    ...
    def read_lines(self, examples):
        ...
        for instance in examples:
            ...
            for evidence in instance['evidence']:
                ids.append(id)
                if type(evidence) is list and len(evidence)>3 and type(evidence[2]) is str:
                    inputs.append([self.process_sent(claim), self.process_sent(evidence[2])])
                else:
                    print('set evidences here')
                evi_list.append(evidence)
        return inputs, ids, evi_list
    ...
    def next(self):
        ...
        if self.step < self.total_step:
            ...
            seg_tensor_input = Variable(
                torch.LongTensor(seg))
            if self.cuda:
                inp_tensor_input = inp_tensor_input.to(self.device)
                msk_tensor_input = msk_tensor_input.to(self.device)
                seg_tensor_input = seg_tensor_input.to(self.device)
            self.step += 1
            return inp_tensor_input, msk_tensor_input, seg_tensor_input, ids, evi_list
        else:
            self.step = 0
